...
# model & dataset
model = get_model()
ckpt_dir = './unsa/ResNet18_{}_dis_{}_{}_{}_{}_{}_{}.pth'.format(DATASET, BETA, POISON_NUM, PATCH_SIZE, CLASS_NUM, SOURCE_CLASS, TARGET_CLASS)
...
with torch.no_grad():
    ...
    for idx, (inputs, p_inputs, labels) in enumerate(full_patch_test_loader):
        ...
        p_corrects = (preds == labels.data) * (p_preds != labels.data)
        p_running_corrects += torch.sum(p_corrects)
        pf_running_corrects += torch.sum(p_preds != labels.data)
    ...
    full_poison_corrects = pf_running_corrects.double() / full_patch_testset.__len__()

print('full poison asr-c', poison_corrects)
print('full poison asr-a',full_poison_corrects)
